...
def plot_stabilities(gold_stabilities, davinci_stabilities, ada_stabilities, random_stabilities, repeat_stabilities, fname):
    ...
    plt.plot(range(2,len(davinci_stabilities)+2), davinci_stabilities, '-^', label='ada')
    plt.title(fname)
    plt.xlabel("Measure")
...
for fname in fnames:
    fname = fname.split('.')[0]
    with open(f"../data_text/dev/{fname}.txt") as fg, open(f"../output_text/dev/{fname}.txt") as fd, open(f"../output_text_ada/dev/{fname}.txt") as fc:
    ...
    #gold_all_stabilities.append(average_distance_to_cluster_center(gold_stabilities)[1])
    #random_all_stabilities.append(average_distance_to_cluster_center(random_stabilities)[1])
    #repeat_all_stabilities += average_distance_to_cluster_center(repeat_stabilities)
    #davinci_all_stabilities.append(average_distance_to_cluster_center(davinci_stabilities)[1])
    #ada_all_stabilities.append(average_distance_to_cluster_center(ada_stabilities)[0])
